...
int Partition(int arr[],int ls,int rs){                  //This method does part of two which has left part that less thaan arr[trmp] and right part that greater than arr[temp]
...
int last=rs;
while(start<last){
    while(arr[temp]>=arr[start]){                       //left part side implementation
        start++;
    }
    while(arr[temp]<arr[last]){                        //right part side implementation
